...
BEGIN
	...
	BEGIN TRY
		...
		TRUNCATE TABLE bronze.crm_cust_info; -- make the table empty then load.
		PRINT '>> Inserting Data Into: bronze.crm_cust_info';
		BULK INSERT bronze.crm_cust_info -- Fullload
		FROM 'D:\sql-data-warehouse-project\datasets\source_crm\cust_info.csv'
		WITH (
			FIRSTROW = 2, -- Skip the first row (header contains columns names)
			FIELDTERMINATOR = ',', -- Dilemeter is comma.
			TABLOCK -- option to improve the performance, lock the whole table while filling it.
		);
